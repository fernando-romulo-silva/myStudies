...
public class MovieClientBean {
	...
	target = client.target("http://" + httpServletRequest.getLocalName() + ":" + httpServletRequest.getLocalPort() + "/" + httpServletRequest.getContextPath() + "/webresources/movie/");
    }
	...
	Movie m = target.path("{movie}").resolveTemplate("movie", bean.getMovieId()).request(MediaType.APPLICATION_JSON).get(Movie.class);
	return m;
    }
