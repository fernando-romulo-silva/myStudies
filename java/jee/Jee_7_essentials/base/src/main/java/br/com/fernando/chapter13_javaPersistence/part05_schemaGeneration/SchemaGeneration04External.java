...
public class SchemaGeneration04External {
	...
	public List<Employee> get() {
	    return em.createQuery("SELECT e FROM SchemaGeneration04External$Employee e", Employee.class).getResultList();
	}
    }
	...
	try (final MyEmbeddedJeeContainer embeddedJeeServer = new MyEmbeddedJeeContainer(); //
	    ...
	    war.addMetaInfFiles(EmbeddedResource.add("persistence.xml", "src/main/resources/chapter13_javaPersistence/part05_schemaGeneration/SchemaGeneration04External-persistence.xml"));
	    
	    final Path tempFolder = Paths.get("/tmp");
	    
	    final Path loadSql = Paths.get("src/main/resources/chapter13_javaPersistence/part05_schemaGeneration/SchemaGeneration04External-load.sql");
	    Files.copy(loadSql, tempFolder.resolve("load.sql"), StandardCopyOption.REPLACE_EXISTING);
	    
	    final Path createSql = Paths.get("src/main/resources/chapter13_javaPersistence/part05_schemaGeneration/SchemaGeneration04External-create.sql");
	    Files.copy(createSql, tempFolder.resolve("create.sql"), StandardCopyOption.REPLACE_EXISTING);
	    
	    final Path dropSql = Paths.get("src/main/resources/chapter13_javaPersistence/part05_schemaGeneration/SchemaGeneration04External-drop.sql");
	    Files.copy(dropSql, tempFolder.resolve("drop.sql"), StandardCopyOption.REPLACE_EXISTING);
