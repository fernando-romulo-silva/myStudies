...
public class StubCriminalCaseRepo extends StubAbstractRepo<CriminalCase> implements CriminalCaseRepo {
	...
	records2.values().forEach(set -> set.stream().filter(c -> c.getNumber().equalsIgnoreCase(caseNumber)).findFirst().ifPresent(c -> result[0] = c));
	...
	records2.values().forEach(set -> resultSet.addAll(set.stream().filter(c -> c.getStatus().equals(status)).collect(Collectors.toSet())));
	return resultSet;
    }
