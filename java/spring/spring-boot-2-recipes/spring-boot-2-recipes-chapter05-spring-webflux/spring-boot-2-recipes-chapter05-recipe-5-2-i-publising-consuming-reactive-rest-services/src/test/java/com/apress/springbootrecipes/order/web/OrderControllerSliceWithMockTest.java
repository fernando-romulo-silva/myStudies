...
public class OrderControllerSliceWithMockTest {
	...
	Flux<Order> orders = Flux.generate((SynchronousSink<Order> orderSynchronousSink) -> orderSynchronousSink.next(generator.generate())).take(10);
	when(orderService.orders()).thenReturn(orders);

	webTestClient.get().uri("/orders").exchange().expectStatus().isOk().expectBodyList(Order.class).hasSize(10);
    }
