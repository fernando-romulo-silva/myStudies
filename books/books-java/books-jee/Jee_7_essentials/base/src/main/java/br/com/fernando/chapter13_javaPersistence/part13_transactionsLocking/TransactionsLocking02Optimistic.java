...
public class TransactionsLocking02Optimistic {
    ...
    // At commit time, when the resource is about to be updated in persistent storage, the state of the resource is read from storage again and compared to 
    // the state that was saved when the resource was first accessed in the transaction. 
    //
    ...
    @Table(name = "MOVIE") //
    @NamedQueries({ //
	    @NamedQuery(name = "Movie.findAll", query = "SELECT m FROM TransactionsLocking02Optimistic$Movie m"), //
	    @NamedQuery(name = "Movie.findAllOptimistic", query = "SELECT m FROM TransactionsLocking02Optimistic$Movie m ", lockMode = OPTIMISTIC), //
	    @NamedQuery(name = "Movie.findAllPessimistic", query = "SELECT m FROM TransactionsLocking02Optimistic$Movie m ", lockMode = OPTIMISTIC_FORCE_INCREMENT), //
    })
	...
	public String toString() {
	    ...
	    builder.append("Movie [id=").append(id).append(", name=").append(name).append(", version=").append(version).append("]");
	...
	public void deleteMovieLock(Integer id) {
	    ...
	    // Alternatively, you can obtain a lock on an entity afterward:
	...
	public Movie findMovieLock(Integer id) {
	    ...
	    // You can obtain a optimistic lock on an entity by passing a LockModeType enum value during the find method:
	    Movie result = em.find(Movie.class, id, OPTIMISTIC);
	...
	private void testUpdade() {
		...
		public void run() {
			...
			System.out.println("OptimisticLockException in secondService.findMovieLock " + e.getClass());
		    }
		}
	    });
	    
	    // if secondService begins the operation first, the firstService update will rollback
	    executor.execute(new Runnable() {
		@Override
		public void run() {
	    ...
	    // During commit (and flush), jpa provider checks every database object that has to be updated or deleted, and compares the version number of that object
	    // in the database to the version number of the in-memory object being updated.
	    //
	    // The transaction fails and an OptimisticLockException is thrown if the version numbers do not match, indicating that the object has been modified by
	    // another user (using another EntityManager) since it was retrieved by the current updater.
	}
    }
    
    //
    // Test Update: Movies before: [Movie [id=1, name=The Matrix, version=0], Movie [id=2, name=The Lord of The Rings, version=0], Movie [id=3, name=OTHER APP, version=0], Movie [id=4, name=The Shining, version=0]]
    ...
    // & Done FirstService.updateMovieLock
    //
    // OptimisticLockException in secondService.findMovieLock class javax.ejb.EJBException
    //
    ...
    public static void anotherApplication(String dataBaseUrl) {

	try (Connection conn = DriverManager.getConnection(dataBaseUrl, DATA_BASE_SERVER_LOGIN, DATA_BASE_SERVER_PASSWORD)) {

	    try (final PreparedStatement ps = conn.prepareStatement("UPDATE MOVIE M SET M.NAME = 'OTHER APP' WHERE M.ID = 3")) {

		// start transaction block
		conn.setAutoCommit(false); // default true

		System.out.println("Begin Other app Update");
		...
		// until the end of transaction, the database lock the record
		conn.commit();

		System.out.println("Done Other app Update");
	...
	try (final MyEmbeddedJeeContainer embeddedJeeServer = new MyEmbeddedJeeContainer(); //
		...
		@Override
		public void run() {
		    anotherApplication(dataBaseUrl);
		}
	    });
