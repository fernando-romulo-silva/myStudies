package br.com.fernando.chapter13_javaPersistence.part15_unsynchronizedPersistenceContext;
...
public class UnsynchronizedPersistenceContext {

    // https://www.objectdb.com/java/jpa/persistence/detach
    
    // By creating a persistence context that is of the new type SynchronizationType.UNSYNCHRONIZED, the automatic join of the transaction as well as the propgation 
    // of updates to the resource manager is disabled. 

    @Entity
    @Table(name = "EMPLOYEE_SCHEMA_UNSYNCHRONIZED_PC")
    @NamedQueries({ //
	    @NamedQuery(name = "Employee.findAll", query = "SELECT e FROM UnsynchronizedPersistenceContext$Employee e") //
    })
	...
	private void listEmployees(PrintWriter out, String title, String result) {
	    String msg = title + " (" + bean.get().size() + ") " + result;
	    out.println("<h3>" + msg + "</h3>");
	    System.out.println(msg);

	    for (Employee e : bean.get()) {
		out.println(e.name + "<br>");
	...
	try (final MyEmbeddedJeeContainer embeddedJeeServer = new MyEmbeddedJeeContainer(); //
	    ...
	    war.addMetaInfFiles(EmbeddedResource.add("persistence.xml", "src/main/resources/chapter13_javaPersistence/part15_unsynchronizedPersistenceContext/persistence.xml"));
	    war.addMetaInfFiles(EmbeddedResource.add("load.sql", "src/main/resources/chapter13_javaPersistence/part15_unsynchronizedPersistenceContext/load.sql"));
	    war.addMetaInfFiles(EmbeddedResource.add("create.sql", "src/main/resources/chapter13_javaPersistence/part15_unsynchronizedPersistenceContext/create.sql"));
	    war.addClasses(ServletPrincipal.class, Employee.class, EmployeeBean.class);
