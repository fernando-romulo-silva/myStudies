...
import br.com.fernando.server.application.security.RSAPublica;
...
public class UsuariosService implements UsuariosServiceInterface {
    ...
    public Response find(final String login, final Date modifiedSince, final RSAPublica chaveCriptografica) {
        Usuario usuario;
        try {
            usuario = em.createNamedQuery("usuario.encontrar.login", Usuario.class).setParameter(1, login).getSingleResult();
    ...
    private void criptografarSenhaUsuario(final Usuario usuario, final RSAPublica chave) {
        try {
