...
class TableLister implements ApplicationRunner {
	...
	try (var con = dataSource.getConnection(); var rs = con.getMetaData().getTables(null, null, "%", null)) {
	    while (rs.next()) {
...
class CustomerLister implements ApplicationRunner {
    ...
    @Override
    public void run(ApplicationArguments args) throws Exception {
	try (var con = dataSource.getConnection(); var stmt = con.createStatement(); var rs = stmt.executeQuery("SELECT id, name, email FROM customer")) {
	    while (rs.next()) {
