...
public class SingerHandlerTest {
	...
	request = ClientRequest.method(HttpMethod.POST, uri).body(BodyInserters.fromObject(singer)).build();
	...
	Flux<Singer> singers = exchange.exchange(request).flatMapMany(response -> response.bodyToFlux(Singer.class));
	...
	ClientRequest request = ClientRequest.method(HttpMethod.POST, uri).body(BodyInserters.fromObject(singer)).build();
