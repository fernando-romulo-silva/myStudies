...
public class DynamicQueriesTest {
	...
	initializeData();
	// tag::adocQueryParam[]
	TypedQuery<Customer> typedQuery = em.createQuery("SELECT c FROM Customer c WHERE c.firstName = :fname", Customer.class);
	...
	initializeData();
	// tag::adocQueryMax[]
	TypedQuery<Customer> typedQuery = em.createQuery("SELECT c FROM Customer c ORDER BY c.age", Customer.class);
