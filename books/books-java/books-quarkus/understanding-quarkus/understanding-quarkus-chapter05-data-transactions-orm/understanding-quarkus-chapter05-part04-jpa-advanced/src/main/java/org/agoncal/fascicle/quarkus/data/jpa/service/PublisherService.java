...
public class PublisherService {
	...
	Publisher publisher = em.createQuery("SELECT p FROM Publisher p WHERE p.name = :name", Publisher.class).setParameter("name", name).getSingleResult();
	return publisher != null ? Optional.of(publisher) : Optional.empty();
    }

    @Transactional(REQUIRED)
    public int deleteByName(String name) {
	int rowsDeleted = em.createQuery("DELETE FROM Publisher p WHERE p.name = :name ").setParameter("name", name).executeUpdate();
	return rowsDeleted;
    }
    // end::adocSkip[]
}
